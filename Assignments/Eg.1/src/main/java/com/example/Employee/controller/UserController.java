...
public class UserController {
	...
	@Autowired
	UserService userService; //interface object
    ...
    public ResponseEntity<User> getUserById(@PathVariable("id") int id) {
	 ...
	 public ResponseEntity<Void> createMultiUser(@RequestBody List<User> user, UriComponentsBuilder ucBuilder){
	     userService.createMultiple(user);
...
//	     headers.setLocation(ucBuilder.path("/user/{id}").buildAndExpand(user.getId()).toUri());
	...
	@GetMapping(value="/highsalary", headers ="Accept=application/json")
	public User highSal(){
		
		List<User> all=getAllUser();
		
		User ans= userService.highSalary(all);
